...
const NAV = [
  ...
  { label: "Villa",       icon: P.villa,   href: "/dashboard/manager/villa" },
  { label: "Bookings",    icon: P.cal,     href: "/dashboard/manager/bookings" },
  { label: "Messages",    icon: P.msg,     href: "/dashboard/manager/messages" },
  { label: "Petty Cash",  icon: P.cash,    href: "/dashboard/manager/petty-cash" },
  { label: "Maintenance", icon: P.wrench,  href: "/dashboard/manager/maintenance" },
  { label: "Services",    icon: P.service, href: "/dashboard/manager/services" },
...
export default function ManagerSettingsPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (!prof || prof.role !== "villa_manager") { window.location.href = "/"; return; }
      setUserId(user.id);
      setName(prof.full_name ?? "");
      setEmail(prof.email ?? user.email ?? "");
      setPhone(prof.phone ?? "");
      const { data: villa } = await supabase.from("villas").select("name").eq("manager_id", user.id).single();
      setVillaName(villa?.name ?? "Not assigned");
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#F5F0E8", fontFamily: "Inter,sans-serif" }}>
      ...
      <aside style={{ width: 220, background: "#2C1E0F", display: "flex", flexDirection: "column", padding: "20px 12px", gap: 2, flexShrink: 0 }}>
        ...
        {NAV.map(item => {
          const isActive = item.href !== null && (item.href === "/dashboard/manager" ? path === item.href : path.startsWith(item.href));
          return (
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "28px" }}>
          <div style={{ maxWidth: 640 }}>
            ...
            <div style={{ background: "linear-gradient(135deg,#2C1E0F,#4A3B27)", borderRadius: 16, padding: "24px 28px", marginBottom: 20, display: "flex", alignItems: "center", gap: 20 }}>
              ...
              <div>
                <div style={{ fontFamily: "Playfair Display,serif", fontSize: 18, fontWeight: 700, color: "#fff" }}>{name || "Manager"}</div>
                <div style={{ fontSize: 12, color: "#C9A84C", marginTop: 2 }}>Villa Manager</div>
                <div style={{ fontSize: 12, color: "#A0906E", marginTop: 1 }}>📍 {villaName}</div>
              </div>
            </div>
            ...
            <div style={{ background: "#fff", border: "1px solid #EDE6D6", borderRadius: 16, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12.5, color: "#9E8E6A", marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid #F0EAD9" }}>Update your name and contact details</div>
            ...
            <div style={{ background: "#fff", border: "1px solid #EDE6D6", borderRadius: 16, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12.5, color: "#9E8E6A", marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid #F0EAD9" }}>Use a strong password of at least 8 characters</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #EDE6D6", borderRadius: 16, padding: "24px 28px", marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12.5, color: "#9E8E6A", marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid #F0EAD9" }}>Choose which events you want to be notified about</div>
              <Toggle checked={notifs.newBooking}        onChange={v => setNotifs(p => ({ ...p, newBooking: v }))}        label="New booking at your villa" sub="Notify when a new booking is confirmed" />
              <Toggle checked={notifs.guestCheckIn}      onChange={v => setNotifs(p => ({ ...p, guestCheckIn: v }))}      label="Guest check-in reminders" sub="24 hours before guest arrival" />
              <Toggle checked={notifs.expenseApproved}   onChange={v => setNotifs(p => ({ ...p, expenseApproved: v }))}   label="Expense approved or rejected" sub="When admin reviews your petty cash submissions" />
              <Toggle checked={notifs.maintenanceUpdate} onChange={v => setNotifs(p => ({ ...p, maintenanceUpdate: v }))} label="Maintenance status updates" sub="When maintenance job status changes" />
              <Toggle checked={notifs.newMessage}        onChange={v => setNotifs(p => ({ ...p, newMessage: v }))}        label="New guest message" sub="When a guest sends a message" />
              <Toggle checked={notifs.dailyBriefing}     onChange={v => setNotifs(p => ({ ...p, dailyBriefing: v }))}     label="Daily villa briefing" sub="Morning summary of upcoming arrivals and tasks" />
